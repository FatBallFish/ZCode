...
function buildWorkspaceHookCandidatePaths(directories: readonly string[]): string[] {
  return directories.flatMap((directory) => [
    ...
    join(directory, ".mikiko", "config.json"),
  ]);
}
...
export function createWorkspaceHookSourceInput(input: {
  ...
  return {
    canonicalPath,
    baseDir: basename(configDirectory) === ".mikiko" ? dirname(configDirectory) : configDirectory,
    ...
    editable:
      !explicitProjectConfig &&
      canonicalPath === resolve(input.workingDirectory, ".mikiko", "config.json"),
    hooks: input.hooks,
  };
}
