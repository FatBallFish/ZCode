...
async function main() {
  ...
  // 未签名 Mac 包注入「安装必读」说明文件（specs/desktop/dmg-installer.md）。
  ...
  // 两处口径若漂移，会出现“已签名包仍带绕 Gatekeeper 说明”或“未签名包缺说明”。
  ...
  if (os === "mac" && !isMacSignedPackaging) {
    console.log("[bundle] 未检测到 Developer ID 签名配置，走无证书打包模式：注入 DMG 安装说明");
