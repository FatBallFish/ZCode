...
// 未签名 DMG 辅助脚本注入器（specs/desktop/dmg-installer.md 的唯一写入路径）。
// electron-builder 26 的 DmgOptions 不支持向 DMG 根目录添加额外文件，这里在打包完成后
// 用 hdiutil 的 shadow 机制把「清除隔离标记」辅助脚本写入只读 UDZO 镜像：
// attach -shadow 挂载 → 拷入脚本 → detach → convert -shadow 合并回 UDZO。
...
const helperSourcePath = resolve(desktopRoot, "build", "dmg-helper-clear-quarantine.command");
const helperVolumeName = "拖入Applications后双击我.command";
...
function detachVolume(mountPoint, label) {
...
function main() {
  ...
  try {
    ...
    // 注意：shadow 挂载必须以可写模式进行（不能加 -readonly），否则脚本拷贝不会进入 shadow 层。
    run("hdiutil", [
      "attach",
      dmgPath,
      "-shadow",
    ...
    try {
      const helperTargetPath = join(mountPoint, helperVolumeName);
      copyFileSync(helperSourcePath, helperTargetPath);
      // .command 在 DMG 内必须保留执行位，Finder 双击才会交给 Terminal 执行。
      chmodSync(helperTargetPath, 0o755);
    } finally {
      detachVolume(mountPoint, dmgPath);
    }
    ...
    // 合并 shadow 后的镜像必须先复验脚本确实在卷内，再覆盖原产物；
    ...
    try {
      ...
      if (!existsSync(injectedHelper)) {
        throw new Error("inject-unsigned-dmg-helper: 注入后复验失败，卷内缺少辅助脚本");
      }
      if (!(statSync(injectedHelper).mode & 0o111)) {
        throw new Error("inject-unsigned-dmg-helper: 注入后的辅助脚本缺少执行位");
      }
    } finally {
      detachVolume(verifyMountPoint, convertedPath);
    }
    ...
    console.log(`[inject-unsigned-dmg-helper] 已注入未签名 DMG 辅助脚本: ${dmgPath}`);
  } finally {
